...
use super::common::{ByteSlice, is_8digits};
...
impl DecimalSeq {
    ...
    ///
    /// For an IEEE 754 binary128 float, this required 11563 digits. So we store the max digits + 1.
    ///
    ...
    const MAX_DIGITS: usize = 11564;

    /// The max decimal digits that can be exactly represented in a 128-bit integer.
    pub(super) const MAX_DIGITS_WITHOUT_OVERFLOW: usize = 38;
    pub(super) const DECIMAL_POINT_RANGE: i32 = 32767;
    ...
    pub(super) fn round<T: UInt>(&self) -> (T, RoundLoss) {
        ...
        for i in 0..dp {
            ...
            if i < self.num_digits {
                n += T::from(self.digits[i]);
            }
        }

        let mut loss = RoundLoss::Zero;

        if dp < self.num_digits {
            match self.digits[dp].cmp(&5) {
                core::cmp::Ordering::Less => loss = RoundLoss::HalfDown,
                core::cmp::Ordering::Equal => {
                    if self.num_digits > dp + 1 || self.truncated {
                        loss = RoundLoss::HalfUp;
                    } else {
                        loss = RoundLoss::Halfway;
                    }
                }
                core::cmp::Ordering::Greater => loss = RoundLoss::HalfUp,
            }
        } else if self.truncated {
            loss = RoundLoss::HalfDown;
        }
...
pub(super) fn parse_decimal_seq(mut s: &[u8]) -> DecimalSeq {
...
#[cfg(test)]
mod tests {
    use super::{DecimalSeq, parse_decimal_seq};
