...
use super::super::super::diy_float::Fp;
use super::super::{Decoded, MAX_SIG_DIGITS, round_up};
...
static CACHED_POW10: [(u64, i16, i16); 81] = [
    // (f, e, k)
...
fn format_shortest_opt<'a>(
    d: &Decoded,
    buf: &'a mut [u8],
) -> Result<(/*digits*/ &'a [u8], /*exp*/ i16), &'a mut [u8]> {
    let d_mant = u64::try_from(d.mant).unwrap();

    assert!(d_mant > 0);
    assert!(d.minus > 0);
    assert!(d.plus > 0);
    assert!(d_mant.checked_add(d.plus).is_some());
    assert!(d_mant.checked_sub(d.minus).is_some());
    assert!(buf.len() >= MAX_SIG_DIGITS);
    assert!(d_mant + d.plus < (1 << 61)); // we need at least three bits of additional precision
    ...
    loop {
        ...
        if plus1rem < delta1 {
            ...
            return round_and_weed(buf, i, exp, plus1rem, delta1, plus1 - v.f, ten_kappa, 1);
        }
...
pub(in super::super::super) fn format_shortest<'a>(
    d: &Decoded,
    buf: &'a mut [u8],
) -> (/*digits*/ &'a [u8], /*exp*/ i16) {
    use super::dragon::format_shortest as fallback;
    match format_shortest_opt(d, buf) {
        Ok(ret) => ret,
        Err(buf) => fallback(d, buf),
    }
}
...
fn format_exact_opt<'a>(
    ...
    assert!(d_mant > 0);
    assert!(d_mant < (1 << 61)); // we need at least three bits of additional precision
    assert!(!buf.is_empty());
...
///
/// This should be used for most cases.
pub(in super::super::super) fn format_exact<'a>(
    d: &Decoded,
    buf: &'a mut [u8],
    limit: i16,
) -> (/*digits*/ &'a [u8], /*exp*/ i16) {
    use super::dragon::format_exact as fallback;
    match format_exact_opt(d, buf, limit) {
        Ok(ret) => ret,
        Err(buf) => fallback(d, buf, limit),
    }
}

#[cfg(test)]
mod tests {
    use super::super::super::tests as base;
    use super::{
        ALPHA, CACHED_POW10, CACHED_POW10_FIRST_E, CACHED_POW10_LAST_E, GAMMA, cached_power,
        format_exact, format_shortest, max_pow10_no_more_than,
    };
    ...
    fn shortest_sanity_test() {
        base::f64_shortest_sanity_test(format_shortest);
        base::f32_shortest_sanity_test(format_shortest);
        //base::f16_shortest_sanity_test(format_shortest);
        base::more_shortest_sanity_test(format_shortest);
    }

    #[test]
    #[cfg_attr(miri, ignore)] // Miri is too slow
    fn exact_sanity_test() {
        base::f64_exact_sanity_test(format_exact);
        base::f32_exact_sanity_test(format_exact);
        //base::f16_exact_sanity_test(format_exact);
    }

    #[test]
    fn test_to_shortest_str() {
        base::to_shortest_str_test(format_shortest);
    }

    #[test]
    fn test_to_shortest_exp_str() {
        base::to_shortest_exp_str_test(format_shortest);
    }

    #[test]
    fn test_to_exact_exp_str() {
        base::to_exact_exp_str_test(format_exact);
    }

    #[test]
    fn test_to_exact_fixed_str() {
        base::to_exact_fixed_str_test(format_exact);
    }
}
