...
use super::super::super::bignum::{Big32x40 as Big, Digit32 as Digit};
use super::super::estimator::estimate_scaling_factor;
...
fn mul_pow10(x: &mut Big, n: usize) -> &mut Big {
    debug_assert!(n < 512);
...
pub(super) fn format_shortest<'a>(
    d: &Decoded,
    buf: &'a mut [u8],
) -> (/*digits*/ &'a [u8], /*exp*/ i16) {
    ...
    let mut k = estimate_scaling_factor(d.mant + u128::from(d.plus), d.exp);

    // convert `{mant, plus, minus} * 2^exp` into the fractional form so that:
    // - `v = mant / scale`
    // - `low = (mant - minus) / scale`
    // - `high = (mant + plus) / scale`
    let mut mant = Big::from_u64(d_mant);
...
#[cfg(test)]
mod tests {
    use super::super::super::super::bignum::Big32x40 as Big;
    use super::super::super::tests as base;
    use super::{format_exact, format_shortest, mul_pow10};
