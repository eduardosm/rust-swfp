...
pub(super) fn estimate_scaling_factor(mant: u128, exp: i16) -> i16 {
    // 2^(nbits-1) < mant <= 2^nbits if mant > 0
    let nbits = 128 - (mant - 1).leading_zeros() as i64;
...
mod tests {
    ...
    use super::estimate_scaling_factor;
    ...
    fn test_estimate_scaling_factor() {
        ...
        for i in (-1074..972).step_by(step) {
            let expected = (i as f64).exp2().log10().ceil();
